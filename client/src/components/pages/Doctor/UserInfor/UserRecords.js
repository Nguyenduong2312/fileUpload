        ...
        <div>
            ...
            <div className="AcceptedRecord_tag">
                <div className='inforPatient'>
                    <h5>Id record: {user.id}</h5>
                    <h5>Date: </h5>
                </div>  
                
                <div className='recordList'>
                    <div style={{width: '100%', margin: 'auto'}}>{message ? <Message msg={message}  /> : null}</div>

                    {recordList?.map(record => 
                        <RecordTag record = {record} idSession = {id} status = {false} setMessage= {setMessage}/>)}
                </div>

                <div className="button back">
                    <Link to= {`/userDetail/${id}`} style={{textDecorationLine: 'none', color: 'white'}}> Back </Link>
                </div>
            </div>
