    ...
    useEffect(() => {
        fetch('http://localhost:5000/login/user', {
            ...
            .then((requests) => {
                ...
                if (requests.role === 'Doctor') {
                    fetch(
                        `http://localhost:5000/requestRecord/sender/${requests.id}`,
                    )
                        .then((res) => res.json())
                        .then((requests) => {
                    ...
                    fetch(
                        `http://localhost:5000/requestRecord/accepted/sender/${requests.id}`,
                    )
                        .then((res) => res.json())
                        .then((requests) => {
                    ...
                    fetch(
                        `http://localhost:5000/requestRecord/receiver/${requests.id}`,
                    )
                        .then((res) => res.json())
                        .then((requests) => {
                    ...
                    fetch(
                        `http://localhost:5000/uploadRecord/receiver/${requests.id}`,
                    )
                        .then((res) => res.json())
                        .then((records) => {
                        ...
                        {first &&
                            acceptedList.map((request) => (
                                <AcceptedList
                                    ...
                                    status={isViewRecord}
                                    setIsViewRecord={setIsViewRecord}
                                />
                            ))}
                        {second &&
